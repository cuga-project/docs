...
<Callout type="info">
**Architecture:** `CugaSupervisor` is the **Supervisor/Router** in the Orchestrator layer — it delegates across `CugaAgent` instances or remote A2A agents. See [How CUGA works](/docs/start/how-it-works#multi-agent-supervisor-vs-spawn).
</Callout>
...
To **expose this CUGA** so others call you over A2A, see [CUGA as A2A](/docs/run/cuga-as-a2a).

To **call a remote agent** that already speaks A2A, pass an external config in `agents`:
...
<Callout type="info">
Full details, support matrix, and a step-by-step example: [Supervisor Policies](/docs/build/policies/supervisor).
</Callout>
