...
<Callout type="info">
**Architecture:** Skills are a **Capability layer** feature — `SKILL.md` playbooks discovered at startup, loaded on demand via `load_skill`, executed in the **Sandbox**. See [How CUGA works](/docs/start/how-it-works#3-capability-layer).

Ready-made skills: **[cuga-skills](https://github.com/cuga-project/cuga-skills)** (`cuga-skills add …` into `.cuga/skills`). To teach Cursor / Claude Code / Codex / Bob how to *author* CUGA skills, use **[cuga-harness-kit](https://github.com/cuga-project/cuga-harness-kit)** (`cuga-harness-kit init`) — those are IDE skills, not runtime playbooks.
</Callout>
...
In chat, type **`/`** to pick a skill from autocomplete — the same playbooks, invoked by name instead of waiting for the model to call `load_skill`.
...
Uncomment **one** provider block in `.env` (Groq, OpenAI, OpenAI-compatible via `settings.openai.toml` + `OPENAI_BASE_URL`, WatsonX, etc.). See [Model Configuration](/docs/configure/models).
...
<Cards>
  <Card href="https://github.com/cuga-project/cuga-skills" title="cuga-skills" description="Install catalog skills into .cuga/skills" icon={<Download />} />
  <Card href="https://github.com/cuga-project/cuga-harness-kit" title="cuga-harness-kit" description="IDE skills that teach Cursor and Claude to build CUGA" icon={<Sparkles />} />
  <Card href="https://skills.sh" title="skills.sh" description="Browse community skills" icon={<Upload />} />
  <Card href="/docs/configure/models" title="Model config" description="OpenAI, WatsonX, Groq, and more" icon={<Terminal />} />
</Cards>
