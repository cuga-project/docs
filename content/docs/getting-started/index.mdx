...
import { Rocket, Settings, BookOpen, Plug, Save, BarChart3, Shield, Code, Database, Sparkles, Server } from 'lucide-react';
...
<Cards>
  ...
  />
  <Card
    href="/docs/customization/knowledge"
    title="Knowledge"
    description="Ingest documents, cite sources, and query them in chat"
    icon={<Database />}
  />
  <Card
    href="/docs/guides/agent-skills"
    title="Agent Skills"
    description="Reusable SKILL.md playbooks the agent loads on demand"
    icon={<Sparkles />}
  />
  <Card
    href="/docs/customization/manage-publish"
    title="Manage & Self-host"
    description="Draft in the UI, publish a version, deploy with Helm"
    icon={<Server />}
  />
  <Card
  ...
  <Card
    ...
    title="Policies"
    description="Intent Guards, Playbooks, ToolGuard, and human-in-the-loop"
    icon={<Shield />}
  />
  <Card
...
- **Policy system**: Intent Guards, Playbooks, Tool Guide, Tool Approval, Output Formatters, and **ToolGuard** (runtime allow/block on tool calls).
- **Knowledge engine**: Ingest PDFs, Office files, HTML, and Markdown with Docling. Answers can cite sources. Documents can be agent-scoped (shared) or session-scoped (one conversation).
- **Agent skills**: Package domain workflows as `SKILL.md` files. Type `/` in chat to invoke one, or let the agent call `load_skill`.
- **Manage & publish**: `cuga start manager` — draft tools, MCP, LLM, and policies in the UI, then publish a versioned config.
- **Self-host**: Helm chart and deploy scripts for kind, minikube, or a cloud cluster.
...
<Accordions type="single">
  <Accordion title="🛠️ Technology Stack">
    - **Python 3.12+**: Core runtime (`requires-python` is `>=3.10,<3.15`)
    - **UV**: Modern Python package management
    - **FastAPI**: High-performance web framework
    - **Playwright**: Browser automation (with a Chromium-based extension for web/hybrid modes)
    - **OpenAI-compatible LLMs**: OpenAI, WatsonX, Groq, Azure, OpenRouter, MiniMax, Gemini, Ollama, and more
    - **Kubernetes / Helm**: Optional self-host path
  </Accordion>
</Accordions>
...
That opens the default chat demo. Other useful presets:

```bash
cuga start manager          # configure tools, models, and policies, then publish
cuga start demo_knowledge   # upload documents and ask questions
cuga start demo_skills      # agent skills + sandbox
```

See [Control Commands](/docs/customization/cli-sdk) for the full list.
...
CUGA is still early. We are investing in:

- **Richer policy and ToolGuard workflows** — easier authoring, review, and reuse of generated guards across environments
- **Adaptive reasoning** — strategies that shift with task complexity, cost, and latency
- **Production operations** — manage/publish, Helm, and observability for long-lived deployments  
