---
title: 'CUGA as MCP Server'
description: 'Run CUGA as an MCP Server'
icon: 'code'
---
...
<Callout type="info">
To expose CUGA as an **agent peer** (not MCP tools), use [CUGA as A2A](/docs/run/cuga-as-a2a) instead.
</Callout>

This example demonstrates how to run CUGA as a Model Context Protocol (MCP) server, enabling it to be used as a tool by other applications.
...
1. **Clone the CUGA repository:**
   ```bash
   git clone https://github.com/cuga-project/cuga-agent.git
   cd cuga-agent/docs/examples/cuga_as_mcp
   ```
...
3. **Configure LLM Access:**
   - Follow [Models](/docs/configure/models) for LLM setup
   - Copy the environment file: `cp .env.example .env`
   - Add your API key to the `.env` file

4. **Start the API Registry (in a separate terminal):**
   ```bash
