...
Each file can be overridden by environment variables.
...
Each settings file configures six agent roles: **planner**, **chat**, **final_answer**, **code**, **qa**, and **action**. You can set model and parameters independently on each role. Other providers use the same keys with a different `platform` (and `model_name` where the provider requires it).
...
WatsonX, Azure, Groq, OpenRouter, MiniMax, Gemini, Ollama, RITS, LiteLLM, and watsonx Orchestrate files in `src/cuga/configurations/models/` use the same six `[agent.*.model]` keys. Change `platform` and provider-specific fields; do not add extra agent roles.
