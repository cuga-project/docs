...
| `run_receipt` | Boolean | `false` | Attach a per-run token/timing [receipt](/docs/sdk/cuga_agent#run-receipt) to `InvokeResult`. No overhead when off. |
...
See [CugaAgent — sub-agent spawn](/docs/sdk/cuga_agent#sub-agent-spawn-opt-in).
...
Agent skills (`SKILL.md`). See [Agent skills](/docs/guides/agent-skills).
