---
title: Customization Overview
description: Understand how CUGA can be customized
---

import {
    Tabs,
    TabsList,
    TabsTrigger,
    TabsContent,
} from 'fumadocs-ui/components/tabs';

You can customize CUGA's behavior and features through the use of configuration files and environment variables.

## Configuration Structure

By default, CUGA uses configuration files which are stored in the `src/cuga/configuration/` folder, however you can
specify a custom configuration directory via this environment variable:

<Tabs defaultValue="mac">
  <TabsList>
    <TabsTrigger value="mac">Mac</TabsTrigger>
    <TabsTrigger value="windows">Windows</TabsTrigger>
  </TabsList>

  <TabsContent value="mac">
    ```bash
    # Set custom configuration directory
    export CUGA_CONFIGURATIONS_DIR="/path/to/your/configurations"
    ```

  </TabsContent>

  <TabsContent value="windows">
    ```powershell
    # Set custom configuration directory
    setx CUGA_CONFIGURATIONS_DIR "C:\path\to\your\configurations"
    ```

  </TabsContent>
</Tabs>

This allows you to organize multiple configurations in separate folders.

Each configuration folder, contains TOML and MD files that allow to toggle features and customize behavior.

```
src/cuga/configurations/
├── models/
│   ├── settings.litellm.toml      # LiteLLM gateway
│   ├── settings.openai.toml       # OpenAI configuration
│   ├── settings.openai.reasoning-high.toml
│   ├── settings.watsonx.toml      # WatsonX configuration
│   ├── settings.azure.toml        # Azure OpenAI configuration
│   ├── settings.groq.toml         # Groq configuration
│   ├── settings.minimax.toml      # MiniMax
│   ├── settings.google.toml       # Gemini
│   └── settings.ollama.toml       # local Ollama
├── modes/
│   ├── fast.toml              # Fast mode settings
│   ├── balanced.toml          # Balanced mode settings
│   └── accurate.toml          # Accurate mode settings
└── instructions/
    └── default/
        ├── api_planner.md     # Custom API planner instructions
        ├── task_decomposition.md   # Custom task decomposition instructions
        └── code_agent.md  # Custom code agent instructions
```

## Customization topics

The following pages include additional information on specific customzation topics:

-   Configure your [LLM providers](../llm-config) of choice
-   Configure [environment variables and system settings](../environment)
-   Customize [reasoning modes and advanced features](../modes) for optimal performance
-   Configure [CUGA's task execution modes](../task-modes)
-   Specify [Special Instructions](../special-instructions) for different sub agents
-   [Onboard tools](../tools) or integrate with existing tool runtime environments
-   [Start presets and CLI](../cli-sdk)
-   [Manage, publish, and self-host](../manage-publish)
-   [Knowledge Base](../knowledge)
