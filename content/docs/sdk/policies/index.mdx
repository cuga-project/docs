...
The `CugaAgent` and `CugaSupervisor` share a policy system accessed via `agent.policies` or `supervisor.policies`. Use it to enforce security guardrails, guide agent behavior with playbooks, and require human approval for sensitive actions.
