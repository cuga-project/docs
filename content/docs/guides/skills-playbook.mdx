...
Copy the environment template and set your provider:
...
<Tabs defaultValue="openai">
  <TabsList>
    ...
    <TabsTrigger value="groq">Groq (default template)</TabsTrigger>
  </TabsList>

  <TabsContent value="openai">

```bash
# Create .env from the template, then set your OpenAI key and model:
cp .env.example .env
# Edit .env — set OPENAI_API_KEY and uncomment the OpenAI block
```
...
```bash
# .env.example already targets Groq — add your key after copying:
cp .env.example .env
```
